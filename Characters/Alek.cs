...
class Character
{
    public string Name { get; set; } = "Alek";
    ...
    public Character()
    {
        Information = new Dictionary<string, dynamic>
        {
            ...
            { "getAge", (Func<int>)(() => (int)((DateTime.Now - Information["birthdate"]).TotalDays / 365)) },
            { "size", 1 * 4 * 1 },
...
class Program
{
    ...
    {
        Character alek = new Character();
        DisplayCharacterSheet(alek);
    }

    static void DisplayCharacterSheet(Character character)
    {
        if (character == null)
        {
        ...
        // Display occupation
        Console.WriteLine("\nOccupation:");
        Console.WriteLine("- " + character.Occupation.Item1 + ", " + character.Occupation.Item2);
